...
test.describe('Change Session Details Page', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await Promise.all([
      ...
      communitySupport.stubGetReferralInformation(200, inPerson.caseRefId),
  ...
  // IPB-2365 - AC3, AC4
  test.skip('Prepopulated by phone', async ({ page }) => {
    await test.step("given I'm on the Change session details screen", async () => {
      await page.goto(ChangeIcsDetailsPage.url(virtual.caseRefId))
    })
    const changeIcsPage = await ChangeIcsDetailsPage.verifyOnPage(page)
  ...
  test.skip('Prepopulated in person', async ({ page }) => {
    await test.step("given I'm on the Change session details screen", async () => {
      await page.goto(ChangeIcsDetailsPage.url(inPerson.caseRefId))
    })
    const changeIcsPage = await ChangeIcsDetailsPage.verifyOnPage(page)
    await test.step('check date and time is correct', () =>
      Promise.all([
        expect(changeIcsPage.dateInput).toHaveValue('1/2/2026'),
        expect(changeIcsPage.timeHourInput).toHaveValue('9'),
        expect(changeIcsPage.timeMinuteInput).toHaveValue('30'),
        expect(changeIcsPage.timeMeridiemInput).toHaveValue('am'),
      ]))
    await test.step('check how will session take place radios', async () => {
      const radio = changeIcsPage.howSessionTookPlaceRadios.getItem('In-person meeting - probation office')
      await Promise.all([
        expect(radio!.input).toBeChecked(),
        expect(changeIcsPage.whyIsSessionNotInPersonDropDown.input).toHaveValue('Welfare check call with Alice'),
      ])
    })
    await test.step('check how they were informed about session radios', async () => {
