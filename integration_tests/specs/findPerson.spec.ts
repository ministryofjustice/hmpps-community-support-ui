...
test.describe('FindPerson', () => {
  ...
  test('AC3-E2E Journey - should display the found details page when a person is found by CRN', async ({ page }) => {
    ...
    await expect(foundPersonPage.personSummary.rows[4].value).toHaveText('Not available')
    await expect(foundPersonPage.personSummary.rows[5].key).toContainText('Current circumstances')
    await expect(foundPersonPage.personSummary.rows[5].key).toContainText('Last updated: Not available')
    await expect(foundPersonPage.personSummary.rows[5].value).toHaveText('Not available')
    await expect(foundPersonPage.personSummary.rows[6].key).toContainText('Disabilities')
    await expect(foundPersonPage.personSummary.rows[6].key).toContainText('Last updated: Not available')
    await expect(foundPersonPage.personSummary.rows[6].value).toHaveText('Not available')
  })
