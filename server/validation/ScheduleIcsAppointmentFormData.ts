...
const buildDateSchema = (min: Date, max: Date) =>
  z
    ...
    .regex(dateRegex, { error: 'Enter a date in the correct format, like 10/7/2025' })
    .superRefine((data, ctx) => {
      if (data.length === 0) {
        return
      }
...
const inSomewhereElseSchema = z.object({
  ...
  addressPostcode: z
    ...
    .regex(validCharRegex, { error: 'Postcode must only include letters a to z, numbers 0 to 9 or spaces' }),
})
